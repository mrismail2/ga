...
import { setWebTitle } from './src/utils/webTitle';
const { SHELLS, resolveInstitutionShell } = require('./src/domain/institutionRouting');

// set once at module load (before first paint) — never left to a template
// substitution that can silently produce "undefined" in the browser tab.
setWebTitle();
...
function NavWrapper() {
  ...
  if (auth.status === 'signed_in') {
    ...
    // Explicit fallback retained for the platform shell. Keeping the concrete
    // component here also protects the long-standing institution-routing
    // regression test from being weakened by an indirect component alias.
    return (
      <NavigationContainer theme={navTheme}>
        <StatusBar style="dark" />
        <RootNavigator />
  ...
  // 4) marketing landing → sign-in gate
  if (!entered) {
    return (
      <>
        <StatusBar style="dark" />
