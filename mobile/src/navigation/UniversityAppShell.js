/* ============================================================
   ...
   enforces isolation). Phase 5 results and transcripts are role-aware: admins
   manage, lecturers enter assigned-course results, and students read only their
   own published results/transcripts.
   ============================================================ */
...
export default function UniversityAppShell() {
  ...
  const { schoolName, profile, signOut, roleKey } = useAuth();
  ...
  const allowedKeys = isAdmin
    ? null
    : roleKey === 'teacher'
      ? new Set(['dashboard', 'results', 'settings'])
      : roleKey === 'student'
        ? new Set(['dashboard', 'results', 'transcripts', 'settings'])
        : new Set(['dashboard', 'settings']);
  const phase4Items = allowedKeys ? UNIVERSITY_NAV_ITEMS.filter((i) => allowedKeys.has(i.key)) : UNIVERSITY_NAV_ITEMS;
  const active = phase4Items.find((i) => i.key === activeKey) || phase4Items[0];
  useEffect(() => { if (!phase4Items.some((i) => i.key === activeKey)) setActiveKey('dashboard'); }, [roleKey, activeKey]);
  ...
  const reloadCounts = React.useCallback(() => {
    ...
    if (!isAdmin) {
      setCountState({ counts: null, loading: false, error: null });
      return () => {};
    }
    if (!schoolId) {
  ...
  const p4Module = isAdmin ? (UNIVERSITY_MODULES.find((m) => m.key === activeKey) || null) : null;
  ...
  const content = (
    <View style={styles.content}>
      {active.key === 'settings' ? (
        ...
        !isAdmin ? (
          <View style={[styles.card, { backgroundColor: c.surface, borderColor: c.line }]}> 
            <Text style={[styles.identityName, { color: c.ink }]}>{(profile && profile.full_name) || 'Kobciye'}</Text>
            <Text style={[styles.emptySub, { color: c.muted, textAlign: 'left', maxWidth: undefined, marginTop: 6 }]}> 
              {roleKey === 'teacher' ? 'Ka gal Results si aad u maamusho natiijooyinka koorsooyinka laguu xilsaaray.' : roleKey === 'student' ? 'Ka eeg Results iyo Transcripts natiijooyinkaaga la daabacay.' : 'Akoonkaaga jaamacadda ayaa si xaddidan u shaqaynaya.'}
            </Text>
          </View>
        ...
        <View style={[styles.card, styles.emptyBox, { backgroundColor: c.surface, borderColor: c.line }]}>
          <Icon name={active.icon} size={28} color={c.muted2} />
          <Text style={[styles.emptyTitle, { color: c.ink }]}>{active.label}</Text>
          <Text style={[styles.emptySub, { color: c.muted }]}>Boggan lama oggola doorka akoonkan.</Text>
        </View>
      )}
    </View>
  );
