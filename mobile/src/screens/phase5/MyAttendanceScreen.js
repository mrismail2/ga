...
export default function MyAttendanceScreen() {
  ...
  const childOptions = useMemo(() => {
    const map = new Map();
    rows.forEach((r) => { if (!map.has(r.student_id)) map.set(r.student_id, (r.student && r.student.full_name) || 'Ilmo'); });
      ...
      {isParent && childOptions.length > 1 ? (
        <View style={styles.childRow}>
          ...
          {childOptions.map((opt) => (
            <TouchableOpacity key={opt.id} onPress={() => setChild(opt.id)} style={[styles.childChip, { borderColor: child === opt.id ? c.blue : c.line, backgroundColor: child === opt.id ? c.blueSoft : c.surface }]}>
              <Text style={[styles.childTxt, { color: child === opt.id ? c.blue : c.muted }]}>{opt.name}</Text>
            </TouchableOpacity>
          ))}
        </View>
      ) : null}
      ...
      {filtered.map((r) => (
        <View key={r.id} style={[styles.row, { backgroundColor: c.surface, borderColor: c.line }]}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={[styles.meta, { color: c.muted }]}>{[r.student && r.student.full_name, r.session && r.session.class && r.session.class.name, r.session && r.session.subject && r.session.subject.name].filter(Boolean).join(' · ') || 'Xaadiris'}</Text>
            {r.corrected_at ? <Text style={[styles.corrected, { color: c.blue }]}>La saxay</Text> : null}
          </View>
...
const styles = StyleSheet.create({
  ...
  corrected: { fontSize: 10.5, fontWeight: '800', marginTop: 3 },
