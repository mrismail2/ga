...
import { listAccountInvitations, provisionAccount, revokeAccountInvitation, getSchoolLoginSettings, setSchoolLoginCode, setParentLoginPhone, p5FriendlyError } from '../../services/phase5';
...
export default function ProvisioningScreen() {
  ...
  const [schoolCodeEdit, setSchoolCodeEdit] = useState('');
  const [schoolCodeBusy, setSchoolCodeBusy] = useState(false);
  const [phoneRec, setPhoneRec] = useState(null);
  const [phoneValue, setPhoneValue] = useState('');
  const [phoneBusy, setPhoneBusy] = useState(false);

  const { data, loading, error, reload } = useAsyncData(
    async () => ({
      school: await getSchoolLoginSettings(schoolId),
  ...
  const school = (data && data.school) || null;
  const records = (data && data.records) || [];
  const invites = (data && data.invites) || [];
  useEffect(() => { if (school && school.login_code) setSchoolCodeEdit(school.login_code); }, [school && school.login_code]);
  ...
  const saveSchoolCode = async () => {
    if (schoolCodeBusy) return;
    const value = schoolCodeEdit.trim().toUpperCase();
    if (value.length < 4) { setErr('School ID waa inuu leeyahay ugu yaraan 4 xaraf.'); return; }
    setSchoolCodeBusy(true); setErr(''); setSuccess('');
    try { await setSchoolLoginCode(schoolId, value); setSuccess('School ID waa la kaydiyay.'); await reload(); }
    catch (e) { setErr(p5FriendlyError(e)); }
    finally { setSchoolCodeBusy(false); }
  };

  const openPhone = (rec) => { setPhoneRec(rec); setPhoneValue(rec.login_phone_e164 || rec.phone || ''); setErr(''); };
  const saveParentPhone = async () => {
    if (phoneBusy || !phoneRec) return;
    if (!phoneValue.trim()) { setErr('Geli mobile-ka WhatsApp-ka waalidka.'); return; }
    setPhoneBusy(true); setErr(''); setSuccess('');
    try {
      await setParentLoginPhone(schoolId, phoneRec.id, phoneValue.trim(), true);
      setPhoneRec(null); setSuccess('Mobile-ka gelitaanka waalidka waa la kaydiyay.'); await reload();
    } catch (e) { setErr(p5FriendlyError(e)); }
  ...
  return (
    <ModuleScreenFrame title="Akoonnada" subtitle="U samee gelitaan macallimiin/arday/waalid">
      <View style={[styles.accessCard, { backgroundColor: c.surface, borderColor: c.line }]}> 
        <View style={{ flex: 1, minWidth: 0 }}>
          <Text style={[styles.lbl, { color: c.muted }]}>ID-GA DUGSIGA EE GELITAANKA</Text>
          <Text style={[styles.hint, { color: c.muted }]}>Ardayda iyo waalidiintu waxay isticmaalaan koodhkan; UUID-ga gudaha lama tuso.</Text>
          <TextInput value={schoolCodeEdit} onChangeText={setSchoolCodeEdit} autoCapitalize="characters" autoCorrect={false}
            placeholder="SCH-0042" placeholderTextColor={c.muted2}
            style={[styles.input, { backgroundColor: c.bg, borderColor: c.line2, color: c.ink, marginTop: 8 }]} />
        </View>
        <SaveButton onPress={saveSchoolCode} saving={schoolCodeBusy} label="Kaydi School ID" />
      </View>
          ...
          {records.map((rec, i) => {
            ...
            return (
              <View key={rec.id} style={[styles.row, { borderTopColor: c.line, borderTopWidth: i === 0 ? 0 : 1 }]}>
                ...
                {tab.role === 'parent' ? (
                  <TouchableOpacity onPress={() => openPhone(rec)} style={[styles.btn, { borderColor: c.line }]}>
                    <Text style={[styles.btnTxt, { color: c.blue }]}>Mobile</Text>
                  </TouchableOpacity>
                ) : null}
      ...
      <Modal visible={!!phoneRec} transparent animationType="fade" onRequestClose={() => setPhoneRec(null)}>
        <Pressable style={styles.overlay} onPress={() => setPhoneRec(null)}>
          <Pressable style={[styles.sheet, { backgroundColor: c.surface }]} onPress={() => {}}>
            <Text style={[styles.sheetTitle, { color: c.ink }]}>Mobile-ka Waalidka · {phoneRec ? phoneRec.full_name : ''}</Text>
            <Text style={[styles.lbl, { color: c.muted }]}>WHATSAPP MOBILE</Text>
            <TextInput value={phoneValue} onChangeText={setPhoneValue} keyboardType="phone-pad"
              placeholder="+25263XXXXXXX" placeholderTextColor={c.muted2}
              style={[styles.input, { backgroundColor: c.bg, borderColor: c.line2, color: c.ink }]} />
            <Text style={[styles.hint, { color: c.muted }]}>Mobile-kan ayaa loo diraa koodhka Forgot Password-ka waalidka iyo ilmaha ku xiran.</Text>
            <SaveButton onPress={saveParentPhone} saving={phoneBusy} label="Kaydi Mobile" />
          </Pressable>
        </Pressable>
      </Modal>
...
const styles = StyleSheet.create({
  accessCard: { borderWidth: 1, borderRadius: 16, padding: 14, marginBottom: 14, gap: 8 },
