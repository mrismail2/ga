...
export default function ClassDetailScreen({ route, navigation }) {
  ...
  const LIVE_TABS = ['Ardayda', 'Xaadiris', 'Natiijada', 'Lacagta', 'Kiisaska'];
  ...
  useEffect(() => {
    if (!clsSchoolId || !clsClassId) { setRollOrder({ order: [], left: [] }); return undefined; }
    let alive = true;
  ...
  useEffect(() => {
    ...
    return () => { alive = false; };
  }, [clsSchoolId, clsClassId, attDateKey]);
