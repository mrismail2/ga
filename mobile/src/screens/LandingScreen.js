...
/* ═══════════════════ LOGIN MODAL (single-card, form only) ════════
   ...
   brand panel, just the form. A floating logo badge sits above three REAL
   login tabs, and each role receives its own secure Forgot Password flow. */
const METHODS = [
...
function LoginModal({ onClose, onMinistry }) {
  ...
  const [identifierPw, setIdentifierPw] = useState('');
  const [showIdentifierPw, setShowIdentifierPw] = useState(false);
  ...
  const login = async () => {
    ...
    try {
      if (method === 'staff') {
        if (!email.trim() || !pw) throw new Error('Geli email iyo furaha sirta.');
        await signIn(email.trim(), pw);
      } else {
        if (!schoolId.trim() || !studentId.trim() || !identifierPw) {
          throw new Error('Geli School ID, Student ID iyo furaha sirta.');
        }
        await signInWithSchoolIdentifier({
          kind: method,
          schoolCode: schoolId.trim(),
          studentId: studentId.trim(),
          password: identifierPw,
        });
      }
    } catch (e) {
      setErr(e && e.message ? e.message : 'Soo galitaanku wuu fashilmay. Hubi xogtaada.');
    ...
    <>
      ...
      <View style={[m.tabs, { backgroundColor: c.bg, borderColor: c.line }]}> 
        {METHODS.map((item) => {
          ...
          return (
            <TouchableOpacity key={item.key} onPress={() => selectMethod(item.key)}
              style={[m.tab, active && { backgroundColor: c.navy }]} activeOpacity={0.85}>
              <Icon name={item.icon} size={14} color={active ? '#fff' : c.muted} />
              <Text style={[m.tabTxt, { color: active ? '#fff' : c.muted }]}>{item.label}</Text>
            </TouchableOpacity>
          );
        })}
      </View>
      ...
      {method === 'staff' ? (
        ...
        <>
          ...
          <View style={[m.field, { backgroundColor: c.bg, borderColor: c.line }]}> 
            ...
            <TextInput value={schoolId} onChangeText={setSchoolId} placeholder="tusaale: SCH-0042"
              placeholderTextColor={c.muted2} autoCapitalize="characters" autoCorrect={false}
              style={[m.input, { color: c.ink }]} />
          </View>
          <Text style={[m.label, { color: c.muted }]}>
            {method === 'student' ? 'ID-GAAGA ARDAY (STUDENT ID)' : 'ID-GA ARDAY-GAAGA (CHILD STUDENT ID)'}
          </Text>
          <View style={[m.field, { backgroundColor: c.bg, borderColor: c.line }]}> 
            <Icon name="students" size={17} color={c.muted2} />
            <TextInput value={studentId} onChangeText={setStudentId} placeholder="tusaale: HID-000142"
              placeholderTextColor={c.muted2} autoCapitalize="characters" autoCorrect={false}
              style={[m.input, { color: c.ink }]} />
          </View>
          <Text style={[m.label, { color: c.muted }]}>{method === 'student' ? 'FURAHA ARDAYGA' : 'FURAHA WAALIDKA'}</Text>
          <View style={[m.field, { backgroundColor: c.bg, borderColor: c.line }]}> 
            <Icon name="key" size={17} color={c.muted2} />
            <TextInput value={identifierPw} onChangeText={setIdentifierPw} placeholder="••••••••"
              secureTextEntry={!showIdentifierPw} placeholderTextColor={c.muted2}
              style={[m.input, { color: c.ink }]} />
            <TouchableOpacity onPress={() => setShowIdentifierPw((v) => !v)} hitSlop={8}>
              <Text style={[m.showTxt, { color: c.blue }]}>{showIdentifierPw ? 'Qari' : 'Tus'}</Text>
            </TouchableOpacity>
          </View>
        </>
      )}
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={m.kav}>
        <View style={[m.card, { backgroundColor: c.surface }]}> 
          ...
          <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled"
            ...
            {view === 'forgot'
              ? <AuthForgotPasswordPanel initialMethod={method} onBack={() => setView('login')} />
              : LoginForm}
          </ScrollView>
        </View>
