...
/* Student/Parent Forgot Password via WhatsApp OTP. This does NOT replace the
   normal identifier+password login; it only verifies the account owner before
   allowing a new password to be set. The request endpoint is anti-enumeration:
   it returns the same generic message whether or not the supplied identity
   resolves. */
export function requestIdentifierPasswordReset({ kind, schoolCode, fullName = null, className = null, parentPhone }) {
  return invokeFunction('request-identifier-password-reset', {
    kind,
...
export function verifyIdentifierPasswordReset({ challengeId, code, newPassword }) {
  return invokeFunction('verify-identifier-password-reset', {
...
/* Phase 5 teacher/student/parent invitation acceptance. The authenticated
   invitee calls this only after setting their own password. The RPC verifies
   the Auth user/email and atomically links the EXISTING domain record. */
export async function acceptAccountInvite(invitationId = null) {
  const { data, error } = await requireClient().rpc('accept_account_invitation', {
    p_invitation: invitationId || null,
  });
  if (error) {
    const err = new Error(error.message || 'Account invitation could not be accepted.');
    err.code = /no pending invitation/i.test(error.message || '') ? 'not_found' :
      /expired/i.test(error.message || '') ? 'expired' : 'account_invite_accept_failed';
    throw err;
  }
  return data;
}
...
export async function clearMustChangePassword() {
  const { error } = await requireClient().rpc('clear_must_change_password');
  if (error) throw error;
}
