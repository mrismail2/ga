...
export default function useCanonicalRows(table, schoolId, { enabled = true, watch = null } = {}) {
  ...
  const reload = useCallback(async () => {
    if (!enabled || !schoolId) { setLoading(false); return; }
    setError(null);
    try { setRows(await p4List(table, schoolId)); }
    catch (e) { setError(p4FriendlyError(e)); }
    finally { setLoading(false); }
  }, [enabled, schoolId, table]);

  useEffect(() => { setLoading(enabled); reload(); }, [reload, enabled]);
  ...
  }, [enabled, schoolId, table, reload, watch && watch.join(',')]);
