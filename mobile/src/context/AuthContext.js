/* ============================================================
   ...
   The single owner of real-auth state and the live/demo switch. It:
     ...
     • toggles isLiveSupabaseMode so real and demo data never mix
   ...
   Demo mode is preserved deliberately: enterDemoMode() lets a developer/tester
   explore the AsyncStorage prototype without a real account. A real
   authenticated user is ALWAYS live — never demo — so they never see the
   Dugsiga Hidaayada seed data.
   ============================================================ */
...
const AuthContext = createContext({
  status: 'initializing', flow: null, flowType: 'invite', flowError: null, mode: 'demo',
  ...
  enterDemoMode: () => {}, refreshProfile: async () => {},
});
...
export function AuthProvider({ children }) {
  ...
  const [demoActive, setDemoActive] = useState(false); // user explicitly chose demo
  const [session, setSession] = useState(null);
  const [profile, setProfile] = useState(null);
  const [profileStatus, setProfileStatus] = useState('idle');
  const [profileError, setProfileError] = useState(null);
  const [error, setError] = useState(null);
  const demoRef = useRef(false); // sticky mirror of demoActive for the listener
  ...
  useEffect(() => {
    const unsub = onAuthStateChange(async (event, s) => {
      ...
      if (event === 'SIGNED_OUT' || !s) {
        ...
        if (!demoRef.current) setMode('demo');
        setStatus('signed_out');
        return;
      }
      setSession(s);
      if (!demoRef.current) setMode('live');
      try { await loadProfile(); }
      catch (e) {
        if (e && e.code === 'stale_profile_request') return;
  ...
  const setNewPassword = useCallback(async (newPassword, { accept = false } = {}) => {
    ...
    if (accept) {
      try {
        await acceptSchoolInvite(null); // server finds the invite by verified email
      } catch (e) {
        logCallbackDiag('invite_accept_failed');
        // e.code may already be one of the specific accept-school-invite
        // codes (expired/cancelled/already_accepted/email_mismatch/
        // not_found/already_member) — SetPasswordScreen shows the matching
        // full-screen state for those. For anything else, passwordSaved
        // tells it to show the specific "password saved, joining failed"
        // message instead of a generic one — the user must never think
        // nothing happened, and must never be told to make a new account.
  ...
  const value = {
    status, flow, flowType, flowError, mode, demoActive,
