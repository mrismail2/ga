...
export default function GuardianManagementView() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const data = await loadGuardianManagementData();
      const grouped = {};
  ...
  const saveGuardian = async () => {
    ...
    try {
      if (guardianForm.id) await updateGuardian(guardianForm.id, guardianForm);
      else await createGuardian(guardianForm);
  ...
  const saveLink = async () => {
    ...
    setSaving(true); setFormError(null);
    try {
      if (linkForm.id) await updateParentStudentLink(linkForm.id, linkForm);
      else await createParentStudentLink(selected.id, linkForm);
  ...
  const unlink = async (linkId) => {
    if (saving) return;
    setSaving(true); setFormError(null);
    try { await deleteParentStudentLink(linkId); setLinkForm(null); await load(); }
    catch (e) { setFormError(p4FriendlyError(e)); }
    finally { setSaving(false); }
  };
