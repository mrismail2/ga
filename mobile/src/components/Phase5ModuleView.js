...
export default function Phase5ModuleView({ module, navigation }) {
  ...
  useEffect(() => {
    setFormOpen(false); setEditingRow(null); setValues({}); setFkOptions({}); setFormErr(null); setSuccess('');
  }, [schoolId]);
  ...
  const openForm = async () => {
    setFormErr(null); setSuccess(''); setEditingRow(null);
    const init = {};
    module.fields.forEach((f) => { init[f.key] = f.default != null ? String(f.default) : ''; });
    setValues(init); setFormOpen(true);
    await loadFieldOptions();
  };

  const openEdit = async (row) => {
    setFormErr(null); setSuccess(''); setEditingRow(row);
    const init = {};
    module.fields.forEach((f) => {
      const raw = row[f.key];
      init[f.key] = raw == null ? (f.default != null ? String(f.default) : '') : String(raw);
    });
    setValues(init); setFormOpen(true);
    await loadFieldOptions();
  };
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      if (wasEditing && module.update) await module.update(editingRow.id, row);
      else await module.create(row);
      setFormOpen(false); setEditingRow(null);
      setSuccess(wasEditing ? 'Waa la cusboonaysiiyay.' : 'Waa la kaydiyay.');
      await load();
  ...
  return (
    <View>
        ...
        <View style={[styles.list, { backgroundColor: c.surface, borderColor: c.line }]}>
          {rows.map((r, i) => (
            <View key={r.id} style={[styles.row, { borderTopColor: c.line, borderTopWidth: i === 0 ? 0 : 1 }]}>
              ...
              {canCreate && module.update ? (
                <TouchableOpacity onPress={() => openEdit(r)} style={[styles.actBtn, { borderColor: c.line }]}> 
                  <Text style={[styles.actTxt, { color: c.blue }]}>Wax ka beddel</Text>
                </TouchableOpacity>
              ) : null}
      ...
      <Modal visible={formOpen} transparent animationType="fade" onRequestClose={() => { setFormOpen(false); setEditingRow(null); }}>
        <Pressable style={styles.overlay} onPress={() => { setFormOpen(false); setEditingRow(null); }}>
          <Pressable style={[styles.sheet, { backgroundColor: c.surface }]} onPress={() => {}}>
            <View style={styles.sheetHead}>
              <Text style={[styles.sheetTitle, { color: c.ink }]}>{editingRow ? 'Wax ka beddel' : 'Ku dar'} {module.single}</Text>
              <TouchableOpacity onPress={() => { setFormOpen(false); setEditingRow(null); }} hitSlop={10}><Icon name="close" size={18} color={c.muted} /></TouchableOpacity>
            </View>
