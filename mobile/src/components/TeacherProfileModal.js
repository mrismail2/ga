...
export default function TeacherProfileModal({ visible, teacher, onClose, onDelete, live }) {
  ...
  const photoKey = code || name;
  const runPhoto = async (capture) => {
    setBusy(true);
    try {
      const res = capture ? await takeStudentPhotoWithCamera() : await pickStudentImageFromGallery();
      if (res && res.uri) setPhoto(photoKey, res.uri);
    } finally { setBusy(false); }
  };
  const removeTeacherPhoto = () => removePhoto(photoKey);
          ...
          <ScrollView contentContainerStyle={styles.body} showsVerticalScrollIndicator={false}>
            {canEditPhoto ? (
              <View style={styles.photoBtns}>
                <TouchableOpacity style={[styles.photoBtn, { backgroundColor: c.navy }]} onPress={() => runPhoto(true)} disabled={busy}>
                  <Icon name="camera" size={15} color="#fff" strokeWidth={2} />
                  <Text style={styles.photoBtnTxt}>Camera</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.photoBtn, { backgroundColor: c.blue }]} onPress={() => runPhoto(false)} disabled={busy}>
                  <Icon name="download" size={15} color="#fff" strokeWidth={2} />
                  <Text style={styles.photoBtnTxt}>Gallery</Text>
                </TouchableOpacity>
                {photos[photoKey] ? (
                  <TouchableOpacity style={[styles.photoBtnGhost, { borderColor: c.rose }]} onPress={removeTeacherPhoto} disabled={busy}>
                    <Icon name="trash" size={15} color={c.rose} strokeWidth={2} />
                    <Text style={[styles.photoBtnGhostTxt, { color: c.rose }]}>Ka Saar</Text>
                  </TouchableOpacity>
                ) : null}
              </View>
            ) : null}
...
const styles = StyleSheet.create({
  ...
  photoBtns: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 14 },
  photoBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 13, paddingVertical: 9, borderRadius: 11 },
  photoBtnTxt: { color: '#fff', fontSize: 12.5, fontWeight: '700' },
  photoBtnGhost: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 13, paddingVertical: 9, borderRadius: 11, borderWidth: 1.4 },
  photoBtnGhostTxt: { fontSize: 12.5, fontWeight: '700' },
