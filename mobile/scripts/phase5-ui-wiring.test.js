...
/* ---------- 8b. actual Forgot Password runtime uses WhatsApp OTP only for recovery ---------- */
const landingScreen = read('src/screens/LandingScreen.js');
const forgotPanel = read('src/components/AuthForgotPasswordPanel.js');
ok('actual LandingScreen renders the role-aware Forgot Password panel',
  /AuthForgotPasswordPanel/.test(landingScreen) && /setView\('forgot'\)/.test(landingScreen));
ok('normal Student/Parent identifier+password login remains intact',
  /signInWithSchoolIdentifier/.test(landingScreen) && /identifierPw/.test(landingScreen));
ok('Student Forgot Password asks full name + School ID + class + Parent mobile',
  /MAGACA ARDAYGA OO BUUXA/.test(forgotPanel) && /ID-GA DUGSIGA/.test(forgotPanel)
  && /FASALKA HADDA/.test(forgotPanel) && /MOBILE-KA WAALIDKA/.test(forgotPanel));
ok('Parent Forgot Password asks School ID + own mobile',
  /method === 'parent'/.test(forgotPanel) || (/ID-GA DUGSIGA/.test(forgotPanel) && /parentPhone/.test(forgotPanel)));
ok('Forgot Password requests and verifies real identifier reset Edge Functions',
  /requestIdentifierPasswordReset/.test(forgotPanel) && /verifyIdentifierPasswordReset/.test(forgotPanel));
ok('password-reset Edge Functions exist',
  fs.existsSync(path.join(root, '..', 'supabase/functions/request-identifier-password-reset/index.ts'))
  && fs.existsSync(path.join(root, '..', 'supabase/functions/verify-identifier-password-reset/index.ts')));
ok('Forgot Password stores no plaintext OTP in the client', !/otp_hash|service_role/i.test(forgotPanel));
...
ok('a ResultEntry screen exists and calls enterResult on a roster',
  ...
  && /enterScheduledResult/.test(read('src/screens/phase5/ResultEntryScreen.js'))
  && /examRosterForSchedule/.test(read('src/screens/phase5/ResultEntryScreen.js')));
...
ok('an AssignmentSubmissions screen exists with submit + grade',
  exists(submScreen)
  && /submitAssignmentWork/.test(read(submScreen)) && /gradeAssignmentSubmission/.test(read(submScreen)));
...
ok('Phase5ModuleView clears the open form + picked values when the school changes',
  /}, \[schoolId\]\);/.test(moduleView) && /setFormOpen\(false\);[\s\S]{0,80}setValues\(\{\}\)/.test(moduleView));
...
ok('finance wires generateInvoice behind a finance-staff-only control',
  /generateInvoice/.test(finSrc) && /Samee biil/.test(finSrc) && /canManage|isFinanceStaff/.test(finSrc));
...
/* ---------- 11. final workflow completion/security ---------- */
ok('Student submissions use the atomic RPC rather than a direct writable grade row',
  /submitAssignmentWork/.test(read(submScreen)) && /submit_assignment_work/.test(svc));
ok('grading uses the dedicated server RPC',
  /gradeAssignmentSubmission/.test(read(submScreen)) && /grade_assignment_submission/.test(svc));
ok('ResultEntry uses schedule-date roster and attendance state',
  /examRosterForSchedule/.test(read('src/screens/phase5/ResultEntryScreen.js'))
  && /enterScheduledResult/.test(read('src/screens/phase5/ResultEntryScreen.js'))
  && /Maqnaa/.test(read('src/screens/phase5/ResultEntryScreen.js')));
ok('finance wires fee items, adjustments, invoice generation, payments and reversal',
  /createFeeItem/.test(finSrc) && /applyInvoiceAdjustment/.test(finSrc)
  && /generateInvoice/.test(finSrc) && /reversePayment/.test(finSrc));
ok('discipline wires actions, notes, attachments and history',
  /createIncidentAction/.test(read('src/screens/phase5/DisciplineScreen.js'))
  && /createIncidentNote/.test(read('src/screens/phase5/DisciplineScreen.js'))
  && /createIncidentAttachment/.test(read('src/screens/phase5/DisciplineScreen.js')));
ok('notifications wire unread count and persisted mark-all-read',
  /unreadNotificationCount/.test(read('src/screens/phase5/NotificationsScreen.js'))
  && /markAllNotificationsRead/.test(read('src/screens/phase5/NotificationsScreen.js')));
