...
import { hmacHex } from "../_shared/whatsapp.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const ipHash = await hmacHex(`identifier-ip:${clientIp(req)}`);
    ...
    // 1. Atomically reserve this attempt. The database serializes concurrent
    // requests for the same identifier/IP, so brute-force calls cannot race
    // past independent count queries.
    const { data: attemptId, error: attemptError } = await admin.rpc("begin_identifier_login_attempt", {
      p_school_code: schoolCode, p_identifier: studentId, p_kind: kind, p_ip_hash: ipHash,
    });
    if (attemptError) {
      console.error(`[${correlationId}] identifier-login: rate_limit_unavailable`);
      return fail(req, 503, "service_unavailable", "Adeegga gelitaanka hadda ma shaqaynayo. Isku day mar kale.", correlationId);
    }
    if (!attemptId) {
    ...
    if (resolveErr || !email) {
      await admin.rpc("complete_identifier_login_attempt", { p_attempt: attemptId, p_success: false });
    ...
    await admin.rpc("complete_identifier_login_attempt", { p_attempt: attemptId, p_success: true });
