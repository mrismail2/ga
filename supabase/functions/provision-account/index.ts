...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // A generated no-email Student must be usable immediately. Complete the
    // profile/domain link server-side now; otherwise students.profile_id stays
    // NULL and identifier-login can never resolve the credentials just issued.
    // The service-only RPC validates the invitation, school, Auth user, and
    // existing Student row atomically and marks the invitation accepted.
    if (needsGeneratedLogin && authUserId && invId) {
      const { error: activateErr } = await admin.rpc("activate_generated_student_account", {
        p_invitation: invId,
        p_auth_user: authUserId,
      });
      if (activateErr) {
        console.error(`[${correlationId}] activate_generated_student_account:`, safeRpcCategory(activateErr.message));
        return fail(req, 500, "activation_failed", "Akoonka waa la sameeyay balse lama hawlgelin. Fadlan isku day mar kale.", correlationId);
      }
    }
