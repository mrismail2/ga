...
(async () => {
  ...
  ok('student submits work',
    await allows(() => db.query(`insert into assignment_submissions (school_id,assignment_id,student_id,content) values ('${schoolA}','${assignId}','${studentRec}','my work')`)));
  ok('a second submission for the same attempt is rejected',
    await rejects(() => db.query(`insert into assignment_submissions (school_id,assignment_id,student_id,content,attempt) values ('${schoolA}','${assignId}','${studentRec}','again',1)`), /duplicate|unique/i));
  await as(teacherU);
  ok('teacher grades the submission',
    await allows(() => db.query(`update assignment_submissions set status='graded', score=8, graded_by='${teacherU}' where assignment_id='${assignId}' and student_id='${studentRec}'`)));
  await as(parentU);
  ...
  ok('exam schedule created',
    await allows(() => db.query(`insert into exam_schedules (school_id,exam_id,class_id,subject_id,exam_date,status) values ('${schoolA}','${examId}','${class1}','${subMath}',current_date+7,'published')`)));
  ok('duplicate exam schedule slot rejected',
    await rejects(() => db.query(`insert into exam_schedules (school_id,exam_id,class_id,subject_id,exam_date,status) values ('${schoolA}','${examId}','${class1}','${subMath}',current_date+7,'draft')`), /duplicate|unique/i));
  // teacher enters a result for an assigned class+subject
  await as(teacherU);
  ok('assigned teacher enters a result', await allows(() => db.query(`select enter_result('${schoolA}','${examId}','${studentRec}',85)`)));
